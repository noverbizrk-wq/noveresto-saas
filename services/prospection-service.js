...
async function searchPlaces(zoneLabel, category, apiKey) {
  ...
  for (const place of results) {
    try {
      const detailUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place.place_id}&fields=name,formatted_phone_number,website,rating,user_ratings_total,formatted_address,geometry&language=fr&key=${apiKey}`;
      ...
      // Un échec de détail individuel ne doit pas casser toute la recherche
      detailed.push({
        google_place_id: place.place_id,
        name: place.name,
 ...
 */
async function searchAndSaveProspects(pool, restaurantId, { zoneLabel, category, userId }) {
  ...
  const places = await searchPlaces(zoneLabel, category, apiKey);
  const saved = [];
